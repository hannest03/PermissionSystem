...
public class DeleteSubCommand extends PermissionBaseSubCommand {
  ...
  public DeleteSubCommand(Plugin plugin, ServiceRegistry serviceRegistry) {
    super("delete", "delete <group>", serviceRegistry, "permission.group.delete");
  ...
  protected void handleCommand(SubCommandSender sender, String[] args) {
    ...
    Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
      ...
      if (group.isDefault()) {
        sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
            language.getTranslation(GROUP_IS_DEFAULT)));
        return;
      }

      permissionService.deleteGroup(group);

      Group defaultGroup = permissionService.getDefaultGroup();
      for (Player player : Bukkit.getOnlinePlayers()) {
        Group playerGroup = permissionService.getPrimaryGroup(player.getUniqueId());
        if (playerGroup == null) {
          permissionService.addPlayerGroup(player.getUniqueId(), defaultGroup);
        }
      }

      sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
          language.getTranslation(GROUP_DELETED)));
    });
  }
